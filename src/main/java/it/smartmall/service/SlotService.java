...
public class SlotService {
    ...
    public List<SlotDTO> getAvailableSlots(Long storeId, LocalDate date) {
        ...
        for (AvailabilityRule rule : rules) {
            for (AvailabilityUtils.OpeningWindow window : AvailabilityUtils.getOpeningWindows(rule)) {
                addSlotsForWindow(availableSlots, bookedSeatsMap, date, rule, window);
            }
        }
    ...
    private void addSlotsForWindow(
            ...
            AvailabilityUtils.OpeningWindow window) {

        LocalTime currentSlotStart = window.start();

        while (currentSlotStart.plusMinutes(rule.getSlotMinutes()).isBefore(window.end()) ||
                currentSlotStart.plusMinutes(rule.getSlotMinutes()).equals(window.end())) {
